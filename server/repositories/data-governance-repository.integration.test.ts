...
const database = createDatabaseClient(process.env.TEST_DATABASE_URL ?? 'postgresql://medibase:medibase_dev@127.0.0.1:55432/medical_supply_manager_test')
...
describe('data governance repository', () => {
  ...
  it('creates organization-scoped assets and audit events', async () => {
    await repository.create({ organizationId, code: 'TEST', name: 'テスト資産', category: 'MASTER', classification: 'INTERNAL', purpose: 'テスト', reviewIntervalDays: 365, nextReviewAt: new Date('2026-08-01T00:00:00Z') })
  ...
  it('records a review and updates the next review atomically', async () => {
    ...
    await repository.review({ organizationId, assetId: asset.id, outcome: 'ACTION_REQUIRED', note: '保持期間を確認', nextReviewAt: new Date('2026-09-01T00:00:00Z') })
    await expect(database.dataAssetReview.count({ where: { dataAssetId: asset.id } })).resolves.toBe(1)
    await expect(database.auditEvent.count({ where: { organizationId, action: 'REVIEWED' } })).resolves.toBe(1)
  })
})

  ...
  return {
    async list(organizationId: string, now = new Date()) {
      ...
      const items = assets.map((asset) => ({
        ...
        nextReviewAt: asset.nextReviewAt.toISOString().slice(0, 10),
        isOverdue: asset.nextReviewAt < now,
        latestOutcome: asset.reviews[0]?.outcome ?? null,
    ...
    async create(input: {
      ...
      if (input.retentionDays !== undefined && input.retentionDays <= 0)
      ...
      return database.$transaction(async (transaction) => {
        ...
        await transaction.auditEvent.create({
          data: {
            ...
            entityId: asset.id,
            action: 'CREATED',
            changedFields: ['code', 'name', 'category', 'classification', 'purpose'],
          },
        })
        return asset
      })
    ...
    async review(input: {
      ...
      return database.$transaction(async (transaction) => {
        const asset = await transaction.dataAsset.findFirst({
          where: { id: input.assetId, organizationId: input.organizationId, archivedAt: null },
        })
        if (!asset) throw new Error('Data asset not found')
        await transaction.dataAssetReview.create({
          data: {
        ...
        await transaction.dataAsset.update({
          where: { id: asset.id },
          data: { lastReviewedAt: new Date(), nextReviewAt: input.nextReviewAt, version: { increment: 1 } },
        })
        await transaction.auditEvent.create({
          data: {
            organizationId: input.organizationId,
            ...
            entityId: asset.id,
            action: 'REVIEWED',
            changedFields: ['lastReviewedAt', 'nextReviewAt'],
          },
        })
      })
    },

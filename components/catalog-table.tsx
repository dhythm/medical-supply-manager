    ...
    <div className="border-border bg-card rounded-md border">
      ...
      <div className="overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="min-w-72">商品</TableHead>
              <TableHead>分類</TableHead>
              <TableHead>コード</TableHead>
              <TableHead className="text-right">定価</TableHead>
              <TableHead className="text-right">契約単価</TableHead>
              ...
              <TableRow key={p.id}>
                <TableCell className="align-top">
                  ...
                  <p className="text-muted-foreground mt-1 text-xs">
                    <span className="font-mono">{p.id}</span> ・ {p.maker}（{p.makerCountry}）
                    {p.usedInEmr ? ' ・ カルテ実績あり' : ''}
                  </p>
                </TableCell>
                <TableCell className="align-top">
                ...
                <TableCell className="text-muted-foreground align-top font-mono text-xs">
                  <p>JAN {p.jan}</p>
                  <p>区分 {p.regulatoryCode}</p>
                  <p>承認 {p.approvalNo}</p>
                </TableCell>
                <TableCell className="align-top text-right font-mono text-xs">
                  ¥{p.listPrice.toLocaleString('ja-JP')}
                </TableCell>
                <TableCell className="align-top text-right font-mono text-xs">
                  ¥{p.contractPrice.toLocaleString('ja-JP')}
                  <span className="text-muted-foreground block">
                    −{Math.round((1 - p.contractPrice / p.listPrice) * 100)}%
                  </span>
                </TableCell>

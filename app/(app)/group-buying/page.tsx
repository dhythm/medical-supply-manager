    ...
    <>
      ...
      <div className="mx-auto grid max-w-[1440px] gap-5 px-5 py-6 md:px-8 md:py-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            label="削減見込"
            value={`¥${result.estimatedSavingsYen.toLocaleString('ja-JP')}`}
            tone="up"
          />
        </div>

        <Panel title="交渉案件を作成">
          <NegotiationCreateForm action={createNegotiationAction} products={options.products} />
        </Panel>
        ...
        <div className="grid gap-5">
                ...
                <header className="border-border/70 flex flex-wrap items-start justify-between gap-3 border-b px-5 py-4">
                  <div>
                    ...
                    <p className="text-muted-foreground mt-1 text-xs">
                      {item.businessCode} ・ {item.productName}
                    </p>
                  </div>
                  <dl className="grid grid-cols-2 gap-x-6 gap-y-1 text-right text-xs sm:grid-cols-4">
                    <div>
                      <dt className="text-muted-foreground">基準</dt>
                      <dd className="font-mono">{yen(item.baselineUnitPriceYen)}</dd>
                    </div>
                    <div>
                      <dt className="text-muted-foreground">最安提示</dt>
                      <dd className="font-mono">{yen(item.bestQuoteYen)}</dd>
                    </div>
                    <div>
                      <dt className="text-muted-foreground">予定数量</dt>
                      <dd className="font-mono">{item.totalQuantity.toLocaleString('ja-JP')}</dd>
                    </div>
                    <div>
                      <dt className="text-muted-foreground">削減見込</dt>
                      <dd className="text-primary font-mono">
                        ¥{item.estimatedSavingsYen.toLocaleString('ja-JP')}
                      </dd>
                    </div>
                  </dl>
                </header>
                    ...
                    content: (
                      <ul className="space-y-2 text-sm">
                        {item.demands.map((demand) => (
                          <li key={demand.facilityId} className="flex justify-between gap-3">
                            <span>{demand.facilityName}</span>
                            <span className="font-mono">{demand.quantity.toLocaleString('ja-JP')}</span>
                          </li>
                        ))}
                        {item.demands.length === 0 ? (
                          <li className="text-muted-foreground">予定数量は未登録です。</li>
                        ) : null}
                      </ul>
                    ),
                    form: !closed ? (
                      <form
                        action={setDemandAction}
                        className="grid items-end gap-2 sm:grid-cols-[minmax(0,1fr)_7rem_auto]"
                      >
                      ...
                      <ul className="space-y-2">
                        {item.quotes.map((quote) => (
                          ...
                          >
                            ...
                            ) : quote.isEligible && !closed ? (
                              <form action={selectQuoteAction}>
                                <input type="hidden" name="negotiationId" value={item.id} />
                                <input type="hidden" name="quoteId" value={quote.id} />
                                <button className="text-primary text-xs font-medium">採択</button>
                              </form>
                            ) : (
                        ...
                        {item.quotes.length === 0 ? (
                          <li className="text-muted-foreground text-sm">提示価格は未登録です。</li>
                        ) : null}
                      </ul>
                    ),
                    form: !closed ? (
                      <div className="grid gap-4">
                        <form
                          ...
                          className="grid items-end gap-2 sm:grid-cols-[minmax(0,2fr)_minmax(7rem,1fr)_minmax(7rem,1fr)_auto]"
                        >
                          <input type="hidden" name="negotiationId" value={item.id} />
                          <label className="grid min-w-0 gap-1 text-xs">
                            卸
                          ...
                          <label className="grid min-w-0 gap-1 text-xs">
                            提示単価
                            <Input name="unitPriceYen" type="number" min={1} required />
                          </label>
                          <label className="grid min-w-0 gap-1 text-xs">
                            最低数量
                            <Input name="minimumQuantity" type="number" min={1} />
                          </label>
                          <Button type="submit" variant="outline">
                            追加
                          </Button>
                        ...
                        {selectedQuote ? (
                          <form
                            action={completeNegotiationAction}
                            className="border-primary/20 bg-primary/5 flex flex-wrap items-end gap-2 rounded-lg border p-3"
                          >
                            <input type="hidden" name="negotiationId" value={item.id} />
          ...
          {result.items.length === 0 ? (
            <Panel title="交渉案件">
              <p className="text-muted-foreground text-sm">交渉案件はありません。</p>
            </Panel>
          ) : null}
        </div>
      </div>

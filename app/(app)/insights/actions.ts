...
export async function createDataAssetAction(formData: FormData) {
  const target = await run(async (organizationId, repository) =>
    repository.create({
      ...
      nextReviewAt: date(formData, 'nextReviewAt'),
    }),
  )
  redirect(target)
}
...
export async function reviewDataAssetAction(formData: FormData) {
  const target = await run(async (organizationId, repository) =>
    repository.review({
      ...
      outcome: text(formData, 'outcome') as DataAssetReviewOutcome,
...
async function run(
  action: (
    ...
    repository: ReturnType<typeof createDataGovernanceRepository>,
  ) => Promise<unknown>,
) {
  try {
    const organization = await getCurrentOrganization()
    await action(organization.id, createDataGovernanceRepository(getDatabaseClient()))
    revalidatePath('/insights')
    return '/insights?updated=1'
  } catch (error) {
    return `/insights?error=${encodeURIComponent(error instanceof Error ? error.message : '更新できませんでした')}`
  }
}
...
function number(data: FormData, key: string) {
  ...
  if (!Number.isInteger(result) || result <= 0) throw new Error('日数は1以上で入力してください')
  return result
}
function optionalNumber(data: FormData, key: string) {
  return optionalText(data, key) ? number(data, key) : undefined
}

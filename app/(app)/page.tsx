    ...
    <>
      ...
      <div className="mx-auto grid max-w-[1440px] gap-5 px-5 py-6 md:px-8 md:py-8">
        {summary.hasSampleData ? (
          <div><Badge variant="outline">サンプルデータを表示中</Badge></div>
        ) : null}

        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard label="登録商品" value={summary.totalProductCount.toLocaleString('ja-JP')} />
          <StatCard label="要確認" value={`${summary.reviewProductCount.toLocaleString('ja-JP')}件`} delta="充足率100%未満" tone="warn" />
          <StatCard label="海外調達" value={`${summary.overseasProductCount.toLocaleString('ja-JP')}件`} delta={`${summary.overseasProductRate}%`} />
          ...
          {summary.recentProducts.length > 0 ? (
            <ul className="divide-border divide-y">
              {summary.recentProducts.map((product) => (
                <li key={product.businessCode} className="flex flex-wrap items-center gap-3 py-3.5 first:pt-0 last:pb-0">
                  ...
                  <Badge variant={product.completeness < 100 ? 'secondary' : 'outline'}>{product.completeness}%</Badge>
                </li>
              ))}
            </ul>
          ) : (
